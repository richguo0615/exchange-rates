<template>
  <div>
        ...
        <li :style="{width: itemWidth}" v-for="(rate, index) in rateList">
          <div class="list-item">
            ...
            <div class="flag-content">
              ...
              <span class="w-rate">
                <input @change="rateList[index].Rate = parseFloat($event.target.value)" type="text" :value="rate.Rate">
              </span>
            </div>
          </div>
        </li>
    ...
    <div @click="updateRateList" class="edit-btn">確認</div>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      updateRateList() {
        ...
        console.log("rateList: ", _this.rateList)

        this.$http.post(api, {"Data": JSON.stringify(_this.rateList)}).then(function (res) {
          if (res.status === 204) {
            alert("更新成功!")
